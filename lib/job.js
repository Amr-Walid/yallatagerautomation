...
/**
 * لو نسبة الصفوف التي لديها كود/باركود أعلى من هذا الحد، نتخطّى فهرسة الفئة
 * ونبحث بالكود مباشرة. السبب المُقاس عملياً: صفحة الفهرس (٢٠٠ منتج) تستغرق
 * ~٢٣ ثانية من الباك إند، بينما البحث بباركود واحد ~٢ ثانية ويُرجع نتيجة واحدة
 * دقيقة. ففي فئة بـ٤٧ صفحة تكون الفهرسة أبطأ بكثير وأثقل على السيرفر.
 */
const CODE_RATIO_SKIP_INDEX = 0.7;

/** سقف زمني لبناء الفهرس؛ إن تجاوزناه نكمل بالبحث المستهدف بدل التجمّد */
const INDEX_BUDGET_MS = 90000;
...
class Job {
  ...
  async _run() {
    ...
    this._checkCancel();
    let index = null;
    // معظم الصفوف لها كود/باركود؟ إذن البحث المستهدف أسرع وأخف من فهرسة الفئة كلها.
    const withCode = rows.filter((r) => r.productId).length;
    const codeRatio = rows.length ? withCode / rows.length : 0;
    const useIndex = rows.length >= INDEX_THRESHOLD && codeRatio < CODE_RATIO_SKIP_INDEX;

    if (!useIndex && rows.length >= INDEX_THRESHOLD) {
      this.emit('log', `${withCode} من ${rows.length} صف لديها كود/باركود — سنبحث بالكود مباشرة `
        + `بدل قراءة الفئة كاملة (أسرع بكثير وأخف على السيرفر)`);
    }
    ...
    if (useIndex) {
      ...
      try {
        // حرس زمني: لو الفئة ضخمة والصفحات بطيئة، نتوقف ونكمل بالبحث المستهدف
        // بدل أن تتجمّد المهمة ساعات. (الفئات الكبيرة تستغرق ~٢٣ ثانية للصفحة)
        ...
        if (index.abandoned) {
          this.emit('warn', `الفئة كبيرة جداً (${index.totalPages} صفحة) وقراءتها كاملة ستطول — `
            + `سنكمل بالبحث المستهدف بالكود/الاسم لكل صف`);
          index = null;
        } else {
    ...
    } else if (rows.length < INDEX_THRESHOLD) {
  ...
  /**
   * بحث محمي من فقدان الجلسة.
   *
   * الباك إند يسمح بجلسة واحدة لكل حساب، فأي تسجيل دخول آخر بنفس الحساب (من
   * المتصفح أو أداة أخرى) يُبطل جلسة المهمة. بدون هذا الحرس كان البحث يرجع
   * فارغاً فتُسجَّل الصفوف "لم يُعثر عليه" وهي موجودة فعلاً — نتيجة خاطئة صامتة.
   * هنا نكتشف الحالة، نُعيد تسجيل الدخول مرة واحدة، ثم نعيد المحاولة.
   */
  async _searchGuarded(query) {
    const doSearch = () => this.client.searchTemplates({
      query, categoryId: this.categoryId, page: 1, limit: 20,
    });
    ...
    try {
      ...
      if (!this._isSessionLost(err)) throw err;
      res = null;
    }

    const lost = !res || this._isSessionLost(res.error);
    if (!lost) return res;

    if (this._reloginCount >= 3) {
      throw new YallaApiError('فقدت الجلسة أكثر من مرة — قد يكون الحساب مستخدماً في مكان آخر. '
        + 'أوقفنا التسجيل لتجنّب نتائج خاطئة.');
    }
    this._reloginCount = (this._reloginCount || 0) + 1;
    this.emit('warn', 'انتهت جلسة الموقع (قد يكون الحساب مفتوحاً في مكان آخر) — '
      + `نُعيد تسجيل الدخول ونكمل (${this._reloginCount}/3)`);
    await this.client.login(this.email, this.password);
    return doSearch();
  }

  /** هل الخطأ/الرد يعني أن جلسة الموقع انتهت؟ */
  _isSessionLost(e) {
    if (!e) return false;
    const code = String(e.code || '');
    const msg = String(e.message || '');
    return code === 'marketplace_seller_102'
      || code === 'marketplace_customer_101'
      || /not logged\s*in/i.test(msg)
      || /customer not logged/i.test(msg);
  }
  ...
  async _findTemplate(row, index) {
    ...
    for (const { q, by } of queries) {
      const res = await this._searchGuarded(q);
      ...
      if (by === 'name') {
        ...
        if (short && short !== q && short.length >= 6) {
          const res2 = await this._searchGuarded(short);
   ...
   *    بسعر منتج آخر. لذلك:
   *      • مع الكود: نقبل **التطابق التام للكود** (أو رقم القالب) فقط —
   *        ولا نقبل "النتيجة الوحيدة" مطلقاً (انظر التعليق داخل الدالة).
   *      • مع الاسم: نقبل التطابق التام أو تشابهاً عالياً متميّزاً عن التالي.
  ...
  _pick(templates, row, by) {
    ...
    if (by === 'id' && row.productId) {
      ...
      // المستخدم أدخل رقم القالب نفسه (template_id) — تطابق قاطع
      const byId = list.filter((e) => e.id === want);
      if (byId.length === 1) return { template: byId[0], score: 98, candidates: byId, ambiguous: null };

      // باركود رقمي طويل: مميّز بطبيعته، لكن نطلبه ككود مستخرج لا كجزء من نص
      const digits = String(row.productId).replace(/\D/g, '');
      if (digits.length >= 8) {
        const hit = list.filter((e) => e.codes.has(digits));
        if (hit.length === 1) return { template: hit[0], score: 88, candidates: hit, ambiguous: null };
      }

      // ⚠️ لا نقبل "النتيجة الوحيدة" في البحث بالكود إطلاقاً.
      // سبب حقيقي مرصود: البحث بـ 57002 أرجع نتيجة وحيدة هي القالب 40060
      // «وش جى فينوس كيرف بكسل رقم (2)، 57002 - أسود» — منتج مختلف تماماً،
      // والرقم صادف وجوده داخل اسمه. قبولها سجّل منتجاً خطأ في حساب التاجر.
      // القاعدة: مع الكود لا تطابق إلا بكود مستخرج مطابق أو رقم قالب مطابق.
